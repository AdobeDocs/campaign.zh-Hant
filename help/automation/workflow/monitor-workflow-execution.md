...
此外，實例的管理員可以使用 **審核跟蹤** 要檢查活動和對工作流所做的最後修改，請查看工作流的狀態。 有關詳細資訊，請參閱。
...
您可以配置工作流，以便在出現錯誤時不暫停並繼續執行。 為此，請編輯工作流 **[!UICONTROL Properties]** 在 **[!UICONTROL Error management]** 的 **[!UICONTROL Ignore]** 的上界 **[!UICONTROL In case of error]** 的子菜單。 然後，可以指定進程暫停前可忽略的連續錯誤數。
...
工作流使用大量工作表，這些工作表佔用空間，如果不進行維護，最終會減慢整個平台的速度。 有關資料庫維護的詳細資訊，請參閱此。

的 **資料庫清理** 可通過 **管理>生產>技術工作流** 節點，用於刪除過時的資料以避免資料庫的指數增長。 工作流自動觸發，無需用戶干預。 請參閱  .
...
1. 將資料夾添加到樹後，請確保將其配置為視圖，以便它顯示所有工作流，無論其來源資料夾是什麼。有關如何配置視圖的詳細資訊，請參閱。
