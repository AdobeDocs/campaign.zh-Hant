...
1. 然後選擇資料限制方法。
   ...
   * **[!UICONTROL By data grouping (in %)]**：您可以根據入站母體的指定欄位中的值，使用百分比來設定記錄數限制。 [了解更多](#limit-the-number-of-subset-records-by-data-grouping)。
   * **[!UICONTROL By data distribution]**：如果您的分組欄位有太多值，或是您想避免為每個新的分割活動再次輸入值，Adobe Campaign可讓您設定 **[!UICONTROL By data distribution]** 限制（選用的分散式行銷模組）。 [了解更多](#limit-the-number-of-subset-records-per-data-distribution)。
