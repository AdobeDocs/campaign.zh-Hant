...
1. 進程的配置。 [了解更多資訊](#configure-processes-associated-with-a-service)。

1. 在市場活動級別引用服務提供商。 [了解更多資訊](#associate-a-service-with-a-campaign)。
...
您現在可以將服務模板與每個服務提供商關聯。 在這些模板中，您必須首先確定成本類別和必要時確定相關庫存。 然後，您可以通過成本結構為每個類別建立成本計算規則。 [了解更多資訊](#define-the-cost-structure)。
...
1. 為每種流程類型（通過直接郵件/電子郵件/等方式交付）建立成本類別。 或任務)。 要執行此操作，請按一下 **[!UICONTROL Cost categories]** 的 **[!UICONTROL Add]** 按鈕，然後輸入每個成本類別的參數。
   ...
   * 如有必要，將庫存行與每種成本類型關聯，以便使用的數量將自動與現有庫存相關。
      ...
      >[!NOTE]
      >
      >在 **[!UICONTROL Stock management]** 的下界。 [了解更多資訊](#stock-and-order-management)。
...
1. 關聯服務提供商的庫建立和引用。 [了解更多資訊](#create-a-stock)。

1. 添加庫存行。 [了解更多資訊](#add-stock-lines)。

1. 通知操作員發生警報。 [了解更多資訊](#alert-operators)。
...
1. 輸入股票的標籤，然後從下拉清單中選擇與其關聯的服務提供商。 [了解更多資訊](#create-service-providers-and-their-cost-structures)。
