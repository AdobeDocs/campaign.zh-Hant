...
根據預設，行銷資源會儲存在 **[!UICONTROL MRM > Marketing resources]** Campaign檔案總管的資料夾。
...
1. 將其拖放至「行銷資源」視窗中的檔案，上傳至Campaign伺服器。 您也可以使用 **[!UICONTROL Upload file to server...]** 連結。
...
上傳後，所有Adobe Campaign操作員都可使用行銷資源。 他們可以檢視、製作復本以修改，或更新伺服器上的檔案。
...
使用 **[!UICONTROL Audit]** 按Tab鍵以新增校樣讀取器並定義資源的可用日期。 在此日期之後，它會顯示為 **[!UICONTROL Late]** 狀態。

>[!NOTE]
>
>此 **[!UICONTROL History]** 索引標籤包含資源的下載和更新記錄。 此 **[!UICONTROL Details]** 按鈕可讓您檢視選取的版本。
>
>此 **[!UICONTROL Audit]** 索引標籤可讓您監視對資源執行的任何動作：核准、拒絕核准、相關評論或出版物。

### 鎖定/解除鎖定資源 {#locking-unlocking-a-resource}

建立後，行銷資源儀表板中就會顯示資源，操作員可以編輯和修改資源。

當運運算元開始處理資源時，最佳實務是鎖定該資源，以防止其他運運算元同時修改該資源。 接著會保留資源：該資源仍可存取，但無法由其他運運算元在伺服器上發佈或更新。
...
更新資源後，按一下 **[!UICONTROL Lock]** 按鈕，讓所有操作者均可再次使用。

特殊訊息會通知嘗試存取該訊息的任何操作員：

![](assets/mkt-resource-locked.png)

此 **[!UICONTROL Tracking]** tab表示鎖定資源的操作員名稱。
...
* 如果資源是透過行銷資源建立任務進行管理，則會在任務設定為時執行 **[!UICONTROL Finished]**，只要 **[!UICONTROL Publish the marketing resource]** 方塊中選取的方塊。 [了解更多](creating-and-managing-tasks.md#marketing-resource-creation-task))
...
進一步瞭解中的傳遞大綱 [本節](../campaigns/marketing-campaign-deliveries.md).
...
1. 編輯坯件或建立新坯件。 進一步瞭解中的庫存 [本節](../campaigns/providers--stocks-and-budgets.md#stock-management).

1. 新增庫存行，並選取對應的行銷資源。

   ![](assets/mkt-resource-in-a-stock-line.png)

   您可以透過以下方式編輯所選資源 **[!UICONTROL Edit the link]** 圖示加以選取後，即會顯示在資源的右側。
