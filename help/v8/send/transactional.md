...
1. [建立及設計訊息範本](#create-message-template). 您必須在此步驟將事件連結至訊息。
1. [測試訊息](#test-message-template).
1. [發佈訊息範本](#publish-message-template).

設計和發佈異動訊息範本後，如果觸發了相對應的事件，則會透過PushEvent和PushEvents將相關資料傳送至Campaign [SOAP方法](../send/event-description.md)，並傳送給目標收件者。

## 建立事件類型 {#create-event-types}

若要確保每個事件都可以變更為個人化訊息，您首先需要建立 **事件型別**.

時間 [建立訊息範本](#create-message-template)，則需選取符合要傳送之訊息的事件型別。
...
1. 在訊息範本中，按一下 **[!UICONTROL Preview]** 索引標籤，然後選取 **[!UICONTROL A seed address]** ，位於下拉式清單中。
...
傳送證明的過程與傳送證明的過程相同。 進一步瞭解中的校訂 [本節](../send/preview-and-proof.md).
...
建立訊息範本時<!-- on the control instance--> 完成後，您可以發佈它，這可讓您傳送連結至即時和批次事件的訊息。
...
發佈完成後，要套用至批次和即時型別事件的訊息範本都會在 **[!UICONTROL Administration > Production > Message Center Execution> Default > Transactional message templates]** 資料夾。

![](assets/messagecenter_deployed_model.png)

發佈範本後，如果觸發了相對應的事件，Adobe Campaign<!--execution instance--> 將接收事件、將其連結至交易式範本，並將對應的交易式訊息傳送給每個收件者。
...
發佈訊息範本後 <!--on the execution instances-->，可取消發佈。
