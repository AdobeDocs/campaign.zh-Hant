...
| deliveryPartRetryDelaySeconds | 重試deliveryPart之前的最小延遲。 這是跨程式與跨容器。 延遲的單位為秒。 | 60 | 0：立即重試 | 3600：非常緩慢的重試（每次重試間隔1小時） | 1：讓在忙碌的記錄檔中輕鬆追蹤重試。 |
...
| providerIdThreads | 提供者ID協調的執行緒數目。 每個執行個體有1個執行緒就足夠了。 設為0會停用此容器。 | 1 | 0：在此容器上停用 | 1 | 1 |
