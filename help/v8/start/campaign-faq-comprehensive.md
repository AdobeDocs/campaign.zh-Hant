...
**Campaign新使用者？** 從[快速入門](#getting-started)開始，瞭解基本知識。\
**需要版本說明嗎？** 檢查[升級](#upgrades)版本資訊和升級程式。\
**從v7或Standard移轉？** 如需差異和轉換指引，請參閱[Campaign v8與舊版](#v7-differences)。\
...
瞭解開始使用Adobe Campaign v8所需的基本知識，從安裝和連線到建立您的第一個行銷活動。

+++ 什麼是Adobe Campaign v8？
...
**適用於：**&#x200B;管理跨多個管道和客戶接觸點之複雜、大量行銷活動的企業行銷團隊。

**相關主題：**

[Campaign v8產品說明](https://helpx.adobe.com/tw/legal/product-descriptions/adobe-campaign-managed-cloud-services.html){target="_blank"} | [v8的新增功能](whats-new.md) | [開始使用指南](get-started.md)
...
Campaign提供多種鎖定目標方法，讓您為訊息選取正確的對象：

**目標定位方法：**

* **查詢編輯器** — 使用收件者屬性、行為或人口統計的視覺化篩選器來建立對象
* **清單** — 使用預先定義的靜態或動態收件者清單
* **檔案匯入** — 上傳一次性行銷活動的外部收件者檔案
* **工作流程** — 使用查詢、分割和擴充活動建立複雜的鎖定目標邏輯
* **預先定義的篩選器** — 套用現成可用的篩選器（活躍客戶、訂閱者、VIP）
* 來自Adobe Experience Platform的&#x200B;**區段** — 利用統一的設定檔和即時區段

您可以結合多個條件（位置、購買記錄、參與），並使用排除、交集或聯合來調整您的對象。

**相關主題：**

[在Campaign v8](../audiences/gs-audiences.md)中定義對象| [查詢編輯器](query-editor.md) | [目標對應](../audiences/target-mappings.md)

+++

+++ 如何建立並傳送第一封電子郵件？

在Campaign v8中建立您的第一封電子郵件很簡單。 您從範本開始，選取您的目標對象，使用個人化設計您的內容，使用校樣進行測試，然後傳送。 Campaign提供兩種建立電子郵件的介面：適用於進階使用者的完整功能&#x200B;**使用者端主控台**，以及現代的&#x200B;**Campaign Web UI**，以更快速、更直覺的方式建立電子郵件。

**5個關鍵步驟：**

1. **建立傳遞** — 從電子郵件範本開始或從草稿建立
2. **定義對象** — 使用查詢、清單或工作流程選取收件者
3. **設計內容** — 使用電子郵件設計工具來建立包含個人化欄位的訊息
4. **傳送測試校樣** — 驗證跨裝置和電子郵件使用者端的轉譯與內容
5. **分析並傳送** — 執行傳遞分析以檢查錯誤，然後傳送您的電子郵件
...
若要聯絡Adobe客戶支援，請連線至[Adobe Admin Console](https://adminconsole.adobe.com/overview){target="_blank"}以建立案例或開始聊天工作階段。
...
是。 Campaign v8與Adobe Experience Cloud解決方案緊密整合，提供統一的行銷生態系統。

**重要整合：** Adobe Experience Platform （統一設定檔、即時資料）、Adobe Analytics （效能測量）、Adobe Target （個人化）、Adobe Experience Manager （內容管理）、Adobe Audience Manager （受眾區段）。

**安裝程式：**&#x200B;需要Adobe IMS驗證，已針對Campaign v8受管理的Cloud Services自動設定。
...
Campaign v8明顯改善了效能，但與Campaign Classic v7在架構上有一些差異，尤其是在FFDA部署上。

**主要考量事項：**

* **FFDA架構** — 使用具有不同資料存取模式的雲端資料庫(Snowflake)
* **資料更新** — 應在工作流程中完成，而不是透過直接的資料庫存取
* **批次最佳化** — 已針對批次作業進行最佳化，而非高頻個別更新
* **在FFDA中無法使用** — 調查、行銷資源管理(MRM)、某些特定聯結器

**移轉影響：**&#x200B;使用直接資料庫寫入的自訂程式碼需要重構；API整合可能需要調整成批次處理。

隨著Adobe增強v8，這些限制也跟著改變。 請參閱最新檔案以瞭解目前狀態。
...
瞭解如何檢查您的Campaign版本、瞭解升級流程，並隨時瞭解新發行版本。 Campaign v8受管理的Cloud Services會自動處理升級，並將中斷情形降至最低。
...
[Campaign v8主要功能](whats-new.md) | [Campaign v8架構](../architecture/architecture.md) | [功能矩陣](https://experienceleague.adobe.com/en/docs/campaign-web/v8/start/capability-matrix){target="_blank"} | [護欄和限制](ac-guardrails.md)

+++

+++ 我應該從Campaign Classic v7或Campaign Standard移轉至v8嗎？

Campaign v8是Adobe的平台，適用於需要大量行銷活動（每小時2000萬項作業）、現代Web UI、雲端原生優勢和長期支援的組織。

**主要優點：**

* **適用於Campaign Standard使用者** — 熟悉的Web UI、功能同位（動態報告、REST API、登陸頁面）、順暢的資料移轉
* **適用於Campaign Classic v7使用者** — 雙介面（主控台+ Web UI）、更優異的效能、自動升級、增強型FFDA架構

**如果您：**，請考慮移轉

* 處理大量資料或體驗效能問題
* 想要降低IT開銷和基礎建設管理
* 需要Adobe Experience Cloud/平台整合
* 想要具備自動更新功能的未來防護技術
...
[從Campaign Classic v7到v8](v7-to-v8.md) | [Campaign Standard轉換指南](https://experienceleague.adobe.com/tw/docs/campaign-web/v8/start/acs-migration){target="_blank"} | [功能矩陣](https://experienceleague.adobe.com/en/docs/campaign-web/v8/start/capability-matrix){target="_blank"}
...
Campaign v8強化了大部分的v7/Standard功能，但有些術語和功能有所不同。 您會在下方找到主要變更的摘要。

**重要術語變更(Campaign Standard → v8)：**

* 自訂資源→ **結構描述** |訊息→ **傳遞** |產品使用者→ **操作員**
* 安全性群組→ **操作員群組** |組織單位→ **資料夾許可權**

**Campaign Web UI更新：**

* 收件者→ **設定檔** |種子地址→ **測試設定檔** |傳遞分析→ **傳遞準備**
* 清單→ **對象** |電子郵件預覽→ **模擬內容**

**在v8中無法使用：**

* 內部部署/混合部署（僅限Managed Cloud Services）
* 直接資料庫存取（使用API）
* 手動基礎建設管理（由Adobe管理）

**Campaign Standard使用者的新功能：**

* 動態報告、集中式品牌、REST API、登陸頁面改進、視覺片段

**相關主題：**

[功能矩陣](https://experienceleague.adobe.com/en/docs/campaign-web/v8/start/capability-matrix){target="_blank"} | [v7到v8轉換指南](v7-to-v8.md) | [Campaign Standard到v8轉換](https://experienceleague.adobe.com/tw/docs/campaign-web/v8/start/acs-migration){target="_blank"}
...
測試設定檔（種子地址）可讓您鎖定不符合已定義定位條件的收件者，讓您在傳送給主要對象之前先測試傳送。 透過&#x200B;**[!UICONTROL Seed addresses]**&#x200B;在傳遞屬性中新增或使用&#x200B;**[!UICONTROL Seed addresses]**&#x200B;資料夾。
...
深入瞭解[重複資料刪除活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/deduplication.html){target="_blank"}
...
深入瞭解[排除活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/exclusion.html){target="_blank"}
...
個人化的主旨列會大幅提高開放率。 Campaign可讓您動態插入收件者資料、套用條件式邏輯及測試變數，以最佳化參與。

**Personalization技術：**

* **基本欄位** — 插入名字、姓氏、位置： `"<%@ firstName %>, exclusive offer for you"`
* **條件式內容** — 依區段區分的不同主旨： `"<% if (recipient.gender == 'F') { %>Special offer just for you<% } else { %>Exclusive deal inside<% } %>"`
* **動態資料** — 包含購買記錄、忠誠度點數或帳戶資訊
* **表情符號** — 新增視覺吸引力（請先在電子郵件使用者端進行測試）

**最佳實務：**&#x200B;長度保持在50個字元以下、傳送前測試個人化權杖、使用A/B測試進行最佳化、避免垃圾訊息觸發字詞、包含價值主張或緊迫性。

**相關主題：**

[個人化欄位](../send/personalization-fields.md) | [條件式內容](../send/conditions.md)
...
[開始傳遞能力](../send/about-deliverability.md) | [控制郵件內容](../send/control-message-content.md) | [監視傳遞能力](../send/monitoring-deliverability.md) | [SpamAssassin](../send/spamassassin.md)

+++

+++ 如何確定我是否成功傳遞，以及是否出現錯誤？

**傳送前：**&#x200B;執行傳遞分析、傳送測試校樣、檢閱警告、驗證目標計數。

**傳送期間/傳送後：**&#x200B;監視傳遞儀表板（已傳送、已傳送、已退回、錯誤）、檢查傳遞記錄、追蹤成功/退回率、檢閱隔離的地址。

**最佳作法：**&#x200B;設定警示、使用巨磁碟區的波段、先測試小磁碟區、定期清理收件者資料庫、監視寄件者信譽。

**相關主題：**

[追蹤及監視傳遞](../send/tracking.md) | [傳遞最佳實務](delivery-best-practices.md)
...
進一步瞭解[種子地址指南](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/action-activities/delivery-control.html){target="_blank"}
...
是！ Campaign可讓您執行A/B測試（也稱為分割測試），以透過比較不同變體的效能來最佳化主旨列、內容、傳送者名稱、傳送時間等。

**您可以測試的專案：**

* **主旨列** — 比較不同主旨的開放率
* **內容變化** — 測試不同的版面配置、行動號召或影像
* **寄件者資訊** — 測試寄件者名稱或來自地址影響
* **傳送時間** — 識別最佳傳遞期間
* **Personalization策略** — 比較個人化內容與一般內容

**運作方式：**

1. 建立您的傳遞並定義測試變體（最多3個）
2. 分割您的對象（測試區段通常為10-20%）
3. Campaign傳送變體至測試區段並追蹤效能
4. 成功變體會自動傳送至剩餘的對象（或您手動選取成功者）

**適用於：** Campaign Web UI和使用者端主控台。 Web UI提供更簡易的設定，以及視覺效果比較。

**相關主題：**

[傳遞分析](../send/delivery-analysis.md) | [傳送及追蹤傳遞](../send/send.md)
...
進一步瞭解[型別規則指南](https://experienceleague.adobe.com/docs/campaign/automation/campaign-optimization/campaign-typologies.html?lang=zh-Hant){target="_blank"}
...
[連結追蹤指南](../send/tracking.md) | [追蹤最佳實務](../send/send.md)
...
[傳遞監視](../send/send.md) | [追蹤指南](../send/tracking.md)
...
是。 Campaign提供多種監控工具：工作流程儀表板（即時狀態和錯誤）、工作流程記錄（詳細執行記錄）、熱度圖（將活動和瓶頸視覺化）、稽核軌跡（追蹤修改）和警報（失敗通知）。

若要監視，請開啟工作流程，然後按一下&#x200B;**記錄檔**&#x200B;索引標籤。 失敗的活動會以紅色顯示。

**相關主題：**

[監視工作流程執行](https://experienceleague.adobe.com/en/docs/campaign/automation/workflows/monitoring-workflows/monitor-workflow-execution){target="_blank"} | [工作流程最佳實務](https://experienceleague.adobe.com/docs/campaign/automation/workflows/introduction/workflow-best-practices.html){target="_blank"}
...
[建立工作流程](https://experienceleague.adobe.com/docs/campaign/automation/workflows/introduction/build-a-workflow.html?lang=zh-Hant){target="_blank"} | [工作流程活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/about-activities.html){target="_blank"} | [工作流程最佳實務](https://experienceleague.adobe.com/docs/campaign/automation/workflows/introduction/workflow-best-practices.html){target="_blank"} | [工作流程使用案例](https://experienceleague.adobe.com/docs/campaign/automation/workflows/use-cases/about-workflow-use-cases.html){target="_blank"}

+++

+++ 如何自動執行週期性行銷活動？

使用工作流程搭配&#x200B;**排程器**&#x200B;活動，以自動化定期排程執行的行銷活動 — 每日、每週、每月或自訂間隔。 非常適合歡迎電子郵件、生日訊息、電子報傳送、捨棄購物車提醒和定期報告。

**安裝模式：**

1. **排程器** — 定義頻率（例如「每個星期一上午9點」）
2. **查詢** — 使用動態條件選取目標對象
3. **擴充** （選擇性） — 新增個人化資料
4. **傳遞** — 設定您的訊息（電子郵件、簡訊、推播）
5. **結束** — 工作流程完成並等待下一次排程的執行

**常見自動化行銷活動：**

* **歡迎系列** — 傳送上線電子郵件給新訂閱者的每日工作流程
* **生日電子郵件** — 每日檢查具有生日內容的收件者，傳送個人化訊息
* **重新參與** — 每週將目標鎖定為非作用中使用者，並提供回授優惠
* **電子報** — 已排程每週或每月內容發佈
* **放棄購買** — 識別放棄購買者並傳送訊息的每小時工作流程

**秘訣：**&#x200B;在工作流程中使用&#x200B;**週期性傳遞**&#x200B;型別，分別追蹤每個執行並維護歷史報告。

**相關主題：**

[排程器活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/flow-control-activities/scheduler.html){target="_blank"} | [週期性傳遞](https://experienceleague.adobe.com/docs/campaign/automation/workflows/use-cases/deliveries/sending-a-birthday-email.html){target="_blank"} | [行銷活動自動化](https://experienceleague.adobe.com/docs/campaign/automation/home.html?lang=zh-Hant){target="_blank"}

+++

+++ 如何將資料匯入 Campaign 中？

**方法：**&#x200B;匯入精靈（一次性CSV/TXT）、工作流程型匯入（**[!UICONTROL Data loading (file)]**&#x200B;活動用於具有轉換的複雜/週期性匯入）、REST API （程式化/即時同步）。

**最佳作法：**&#x200B;以小樣本進行測試、使用UTF-8編碼、正確對應欄位、套用重複資料刪除、排程非尖峰的大量匯入。

**相關主題：**

[匯入最佳實務](../start/import.md) | [資料載入活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/action-activities/data-loading-file.html){target="_blank"} | [週期性匯入工作流程](https://experienceleague.adobe.com/docs/campaign/automation/workflows/use-cases/data-management/recurring-import-workflow.html){target="_blank"}

+++

+++ 如何確保匯入期間的資料品質？

資料品質是成功執行行銷活動的關鍵。 不良資料會導致傳送失敗、資源浪費及合規風險。 Campaign提供的工具可在匯入過程中驗證、清除及擴充資料。

**資料驗證步驟：**

1. **預先匯入驗證** — 驗證檔案格式、編碼(UTF-8)、資料行對應、必填欄位是否存在
2. **重複資料刪除** — 使用&#x200B;**[!UICONTROL Deduplication]**&#x200B;活動以電子郵件、識別碼或自訂金鑰識別及處理重複專案
3. **資料擴充** — 使用&#x200B;**[!UICONTROL Enrichment]**&#x200B;活動從現有Campaign資料表中新增遺漏的資料
4. **格式標準化** — 使用JavaScript或擴充將電話號碼、電子郵件地址、日期、國家/地區代碼標準化
5. **驗證規則** — 套用限制（有效的電子郵件格式、必要欄位、值範圍）

**常見問題與修正：**

* **字元編碼錯誤** →永遠使用UTF-8編碼
* **日期格式不符** →標準化為YYYY-MM-DD格式
* **無效的電子郵件地址** →使用驗證規則或JavaScript進行篩選
* **重複記錄** →更新前一律包含重複資料刪除活動
* **缺少必要欄位** →設定預設值或拒絕不完整的記錄

**最佳實務：**&#x200B;建立可重複使用的「資料品質」工作流程範本，其中包含您可以套用至所有匯入的標準驗證和清除活動。

**相關主題：**

[資料品質指南](../start/import.md) | [重複資料刪除活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/deduplication.html){target="_blank"} | [擴充活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/enrichment.html){target="_blank"}
...
**資料管理：**&#x200B;匯入/載入資料、清除、擴充、清單管理\
**行銷活動自動化：**&#x200B;歡迎系列、生日行銷活動、重新參與、購物車放棄\
**進階目標定位：** A/B測試、動態細分、潛在客戶評分、跨頻道協調\
**監視：**&#x200B;工作流程/傳遞監視、警示、資料庫維護\
**整合：** CRM同步、API整合、事件觸發的工作流程
...
使用&#x200B;**[!UICONTROL Update data]**&#x200B;活動進行大量資料庫作業：插入（新增記錄）、更新（修改現有記錄）、插入或更新（更新插入）、刪除（移除比對記錄）。

**常見用法：**&#x200B;更新設定檔屬性、與外部系統同步、維護清單成員資格、清除/刪除重複資料、套用大量狀態變更。

設定調解金鑰以精確比對，並選擇更新選項。

**相關主題：**

[更新資料活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/update-data.html){target="_blank"} | [資料管理活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/action-activities/about-action-activities.html){target="_blank"}

+++

+++ 如何運用資料管理功能？

資料管理活動可啟用複雜操作：擴充（從相關表格新增資料）、分割（區段母體）、重複資料刪除（移除重複專案）、更新資料（大量操作）、變更維度（切換目標維度）、交集/聯合/排除（合併/篩選母體）。

**常見用法：**&#x200B;擴充購買/行為資料、區隔對象、移除重複專案、整合外部資料庫(FDA)、建立複雜的多資料表查詢。

**相關主題：**

[資料管理活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/about-targeting-activities.html){target="_blank"} | [擴充活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/enrichment.html){target="_blank"}

+++

+++ 我可以自動個人化訊息傳送嗎？

是。 建立自動化工作流程：查詢（目標對象） →擴充（新增個人化資料） →分割（選擇性區段） →傳送（個人化訊息） →排程器（循環執行）。

**Personalization：**&#x200B;使用設定檔資料、行為資料、條件式內容、動態值。 常見情境：生日行銷活動、購物車放棄、忠誠計畫、回訪、事件觸發訊息。

**相關主題：**

[Personalization指南](../send/personalize.md) | [工作流程使用案例](https://experienceleague.adobe.com/docs/campaign/automation/workflows/use-cases/deliveries/send-a-birthday-email.html?lang=zh-Hant){target="_blank"}

+++

+++ 如何使用工作流程將受眾分隔到子集中？

使用&#x200B;**[!UICONTROL Split]**&#x200B;活動來劃分母體：篩選條件（年齡、位置、VIP狀態）、百分比分佈（A/B測試）、限制記錄（前N、前X%）、資料分組（每個值一個子集）。

**常見用法：** A/B測試、管道偏好設定路由、漸進式轉出、區段特定訊息、負載平衡。 每個子集流向不同的轉變，以進行不同的處理。

**相關主題：**

[分割活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/split.html){target="_blank"} | [A/B測試指南](https://experienceleague.adobe.com/docs/campaign/automation/workflows/use-cases/deliveries/a-b-testing.html){target="_blank"}

+++

+++ 如何從外部檔案更新收件者資料？

是。 工作流程：資料載入（檔案） →擴充（選用） →調解（比對索引鍵，例如電子郵件/ID） →更新資料（更新比對記錄，如果沒有比對則插入新記錄）。

**常見用法：**&#x200B;從CRM更新設定檔屬性、重新整理訂閱狀態、同步熟客點數、更新選擇加入/選擇退出偏好設定。

**最佳作法：**&#x200B;使用唯一識別碼、先驗證資料、使用範例測試、排程定期更新。

**相關主題：**

[匯入資料指南](../start/import.md) | [資料載入活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/action-activities/data-loading-file.html){target="_blank"} | [更新資料活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/update-data.html){target="_blank"}

+++

+++ 如何識別和鎖定新的收件者？

查詢&#x200B;**[!UICONTROL Created date]**&#x200B;欄位以選取在特定時間範圍內新增的收件者。

**自動歡迎工作流程：**&#x200B;排程器（每日執行） →查詢（選取新收件者） →重複資料刪除（選用） →傳送（歡迎訊息） →更新資料（標示為「已歡迎」）。

**進階：**&#x200B;使用彙總函式來動態識別最近的新增。

**相關主題：**

[查詢活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/query.html){target="_blank"} | [使用彙總](https://experienceleague.adobe.com/docs/campaign/automation/workflows/use-cases/data-management/using-aggregates.html){target="_blank"}
...
**目標：**&#x200B;查詢、分割、重複資料刪除、擴充、交集、聯合、排除（定義/調整對象）\
**流程控制：**&#x200B;排程器，等待，測試，分叉，AND — 連線，OR — 連線，跳轉（管理邏輯/計時）\
**動作：**&#x200B;傳遞、更新資料、資料載入/擷取、JavaScript程式碼（執行作業）\
**事件：**&#x200B;外部訊號、檔案收集器、HTTP傳輸（回應觸發程式）

從浮動視窗拖曳，按兩下以設定、使用切換效果連線。

**相關主題：**

[目標定位活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/targeting-activities.html){target="_blank"} | [流量控制](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/flow-control-activities/flow-control-activities.html){target="_blank"} | [動作活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/action-activities/action-activities.html){target="_blank"}

+++

+++ 什麼是工作流程最佳實務？

**設計：**&#x200B;清除名稱、新增標籤/說明、群組相關活動、將複雜的程式分成較小的工作流程\
**效能：**&#x200B;限制查詢大小、使用暫存資料表、排程非尖峰、避免過多的回圈\
**錯誤處理：**&#x200B;新增錯誤路徑、設定警示、使用範例測試、檢閱記錄檔\
**維護：**&#x200B;封存過時的工作流程、版本控制、限制複雜性（&lt;20個活動）、使用範本\
**安全性：**&#x200B;套用許可權、清除暫存資料、使用非硬式編碼值的變數

**相關主題：**

[工作流程最佳實務指南](https://experienceleague.adobe.com/docs/campaign/automation/workflows/introduction/workflow-best-practices.html){target="_blank"} | [監視工作流程](https://experienceleague.adobe.com/docs/campaign/automation/workflows/monitoring-workflows/monitor-workflow-execution.html){target="_blank"}
...
[控制面板檔案](https://experienceleague.adobe.com/zh-hant/docs/control-panel/using/control-panel-home){target="_blank"} | [控制面板教學課程影片](https://experienceleague.adobe.com/en/docs/control-panel-learn/tutorials/control-panel-overview){target="_blank"}

+++

+++ 如何檢查我的操作員和許可權是否遵循安全性最佳實務？

Adobe Campaign v8提供將頂層管理帳戶和特殊許可權帳戶的所有目前設定與建議的安全預設值比較的功能。

* **目前設定（完整可見度）：**&#x200B;在&#x200B;**[!UICONTROL Administration]** > **[!UICONTROL Access management]** （使用者端主控台或Campaign Web UI）： **[!UICONTROL Operators]**、**[!UICONTROL Operator groups]**、**[!UICONTROL Named rights]** （例如管理、核准管理）和資料夾層級的許可權中，可以檢閱管理帳戶和授權帳戶的所有設定。 產品管理員也可以在[Campaign控制面板](https://experienceleague.adobe.com/docs/control-panel/using/control-panel-home.html?lang=zh-Hant){target="_blank"}中檢閱執行個體層級的設定（IP允許清單、URL許可權、執行個體詳細資料等）。
* **建議的安全預設值：** [安全性准則](../config/security.md)會為管理員和特殊許可權帳戶定義建議的安全基準，包括：建立足夠的安全性群組、為每個操作員指派適當的存取許可權，以及限制管理員操作員和管理員群組的使用。 若適用，請一併參閱[增強式安全性附加元件](../config/enhanced-security.md)，以強化網路與加密。
* **比較：**&#x200B;管理員會比較其目前的設定與記錄建議以找出差距，並符合安全預設值。

**相關主題：**

[開始使用許可權](gs-permissions.md) | [安全性方針](../config/security.md) | [增強式安全性附加元件](../config/enhanced-security.md)
...
[追蹤及監視傳遞](../send/tracking.md) | [設定追蹤的連結](../send/tracked-links.md) | [測試追蹤](../send/testing-tracking.md)
...
[關於行銷活動中的傳遞能力](../send/about-deliverability.md) | [傳遞能力最佳實務指南](https://experienceleague.adobe.com/docs/deliverability-learn/deliverability-best-practice-guide/introduction.html?lang=zh-Hant){target="_blank"}

+++

+++ 如何存取儲存在外部資料庫的資料？

Campaign v8提供同盟資料存取(FDA)功能，可查詢及使用儲存在外部資料庫中的資料，無需變更Campaign資料模型。 您可以設定外部帳戶，然後在工作流程、查詢和定位中使用FDA，將外部資料與Campaign設定檔結合。

FDA可用性和連線需求取決於您的部署和授權。 檢閱相容性矩陣中支援的資料庫和必要版本。

**相關主題：**

[設定FDA連線](../connect/fda.md) | [相容性矩陣](compatibility-matrix.md)

+++

+++ 我可以將 Campaign 連線到哪些外部資料庫？

Campaign v8支援同盟資料存取(FDA)連線到主要企業資料庫系統（雲端資料庫、企業資料庫、資料倉儲、巨量資料平台）。

支援的資料庫版本和連線需求會有所不同。 請檢查您Campaign v8版本的[相容性矩陣](compatibility-matrix.md)，以確認特定資料庫的支援，並確保FDA聯結器獲得適當的授權。

請參閱[設定FDA連線](../connect/fda.md)

+++

+++ Adobe Campaign可以與CRM系統整合嗎？

是。 Campaign提供原生CRM聯結器，用於與主要CRM系統進行雙向同步。 同步聯絡資料、銷售機會、帳戶、傳遞記錄、追蹤資料和參與量度。 支援排程、手動和即時（透過API）同步模式。

使用Campaign的CRM聯結器助理來對應欄位、選取表格並排程同步。 檢查[相容性矩陣](compatibility-matrix.md)以取得支援的CRM版本。
...
關閉使用者端主控台，然後從您的使用者設定檔中手動移除本機快取檔案。

**手動快取清除(Windows)：**

1. 在使用者端主控台中，選取&#x200B;**[!UICONTROL File]** > **[!UICONTROL Clear the local cache...]**。
2. 登出並關閉使用者端主控台。
3. 根據您的Windows版本，移至快取位置。 針對Windows 10： `C:\Users\<Username>\AppData\Roaming\Neolane\NL_5\`
4. 刪除`nlclient-config-<alphanumerical>.xml`檔案及其相關資料夾。

>[!IMPORTANT]
>
>請勿刪除`nlclient_cnx.xml`。

Adobe支援可確認您的版本和作業系統的正確快取位置，並引導您完成安全的手動清理。

深入瞭解[安裝及設定使用者端主控台](connect.md)

+++

+++ Adobe Campaign可以與輕型目錄存取協定(LDAP)整合嗎？

沒有。 Campaign v8不支援LDAP整合。 驗證使用Adobe IMS （Adobe ID或Federated ID）。 如果您要從Campaign Classic v7移轉，請檢閱不支援的功能和驗證變更。

**相關主題：**

[連線至Campaign v8](connect.md) | [v8](v7-to-v8.md#gs-removed)不支援的功能
...
Campaign會根據您的需求和技術專業提供多種報告選項：內建報告、描述性分析、使用者端主控台中的自訂報告，以及多維度資料集。
...
立方體是多維度資料結構，可讓業務使用者透過樞紐分析表探索及分析Campaign資料，而不需要技術技能。 請將它們視為預先設定的資料模型，可簡化複雜的報表。 此報告工具可用於使用者端主控台和Campaign Web UI。
...
**重要：**&#x200B;調查管理在Campaign v8企業(FFDA)部署中無法使用。 [了解更多資訊](../architecture/enterprise-deployment.md)。
...
透過Campaign中的檔案夾許可權和已命名許可權控制報告的可見度。

**存取控制方法：**

* **資料夾許可權** — 將報表放在具有使用者群組適當存取權的資料夾中
* **已命名的許可權** — 指派許可權以檢視、建立或修改報告
* **顯示內容** — 定義報表出現的位置（報表資料夾、行銷活動標籤、傳遞畫面）

**設定：**&#x200B;將報告儲存至特定資料夾→設定操作員群組的資料夾存取權→定義報告屬性和顯示內容。

**相關主題：**

[自訂報表](../reporting/custom-reports.md) | [使用者許可權](gs-permissions.md)
...
[自訂報告](../reporting/custom-reports.md) | [促銷活動Web UI報告](https://experienceleague.adobe.com/en/docs/campaign-web/v8/reports/gs-reports){target="_blank"}
...
Campaign的資料模型是方案導向的關聯式資料庫結構，包含可以根據您的業務需求擴充的內建表格（收件者、傳送、行銷活動）。

**重要概念：**&#x200B;結構描述（XML定義）、內建表格、連結（關係）、列舉（值清單）、副檔名（自訂欄位/表格）。

**主要結構描述：**&#x200B;收件者(`nms:recipient`)、傳遞(`nms:delivery`)、工作流程(`xtk:workflow`)、行銷活動(`nms:operation`)、追蹤記錄。

瞭解資料模型對於工作流程、查詢、結構描述擴充功能和整合至關重要。

**相關主題：**

[Campaign資料模型](../dev/datamodel.md) | [資料模型最佳實務](../dev/datamodel-best-practices.md)

+++

+++ 如何使用 Campaign 綱要？

結構描述會以XML格式定義Campaign的資料結構，指定表格結構、欄位屬性、關係、索引和存取控制。

**使用結構描述：**

* **檢視：**&#x200B;透過&#x200B;**[!UICONTROL Administration > Configuration > Data schemas]**&#x200B;的存取
* **延伸：**&#x200B;建立延伸結構描述（例如`cus:recipient`）以新增自訂欄位而不修改核心結構描述
* **建立：**&#x200B;建立特定企業資料的新資料表
* **更新：**&#x200B;透過&#x200B;**[!UICONTROL Tools > Advanced > Update database structure]**&#x200B;套用變更

**常見用法：**&#x200B;新增自訂欄位至收件者表格、建立自訂表格、定義關係、實作特定商業模型。

**重要：**&#x200B;絕對不要直接修改內建結構描述。 請一律使用擴充功能結構描述來升級相容性。
...
目標B2B帳戶、個別訂閱者資料、外部系統或多品牌架構時，請使用自訂收件者表格，而非標準收件者表格。

**實作：**&#x200B;使用必要欄位（電子郵件、主索引鍵、排除專案）建立自訂結構描述→設定目標對應→更新傳遞範本→調整工作流程/查詢。

**重要考量：**&#x200B;必須包含必要的傳遞欄位，工作流程/表單需要調整，在生產移轉之前必須測試關鍵專案。

**最佳實務：**&#x200B;請先擴充標準收件者資料表。 由於增加複雜性，只有在真正必要時才使用自訂表格。
...
Campaign的查詢編輯器會以視覺化方式建立資料庫查詢，而不使用SQL，用於工作流程活動、傳遞目標定位、清單、報告和篩選器。

**最佳實務：**

* 開始簡單 — 逐步建置，測試每個步驟
* 使用篩選維度 — 運用表格關係
* 最佳化績效 — 為查詢的欄位編制索引，避免複雜的計算
* 重複使用預先定義的篩選器以維持一致性
* 先使用小型樣本進行測試
* 檔案複雜查詢

**常見模式：**&#x200B;目標傳遞開啟者、尋找非作用中連絡人、依行為分段、排除先前的收件者。

**存取：** **[!UICONTROL Tools > Generic query editor]**&#x200B;以進行個人探索。

**相關主題：**

[查詢編輯器](../start/query-editor.md) | [查詢活動](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/query.html){target="_blank"}

+++

+++ 如何匯入資料包？

透過使用者端主控台中的&#x200B;**[!UICONTROL Tools > Advanced > Import package]**&#x200B;匯入套件。 套件包含用於在執行個體之間部署的Campaign設定（結構、工作流程、型別）和資料。

**封裝型別：**&#x200B;使用者封裝（自訂設定）、平台封裝（Adobe提供）、資料封裝（實際資料）。

**最佳實務：**&#x200B;請先在開發環境中測試，匯入之前先備份，然後從相同/較舊的版本匯出。

深入瞭解[使用資料套件](../dev/packages.md)

+++

+++ 我可以在哪裡找到Campaign v8 API清單？

Campaign v8提供SOAP API （使用者端主控台作業）、REST API （現代化整合）和JavaScript API （工作流程指令碼）。

**常見用法：**&#x200B;與CRM/ERP整合、自動化行銷活動、同步資料、建置監控解決方案、建立外部介面。
...
Campaign API可讓您以程式設計方式控制工作流程：開始、暫停/繼續、停止、查詢狀態、擷取記錄、監視活動進度。

**API端點：** `POST https://mc.adobe.io/<ORGANIZATION>/campaign/workflow/execution/<workflowID>/commands`

**命令：** `{"method":"start"}`，`{"method":"pause"}`，`{"method":"resume"}`，`{"method":"stop"}`

**使用案例：**&#x200B;建置監視儀表板、實作自動警示、從外部排程器協調、建立執行個體之間的相依性、產生自訂報告。

**最佳實務：**&#x200B;將API監視與稽核軌跡結合，以進行全面治理。

檢視[透過API控制工作流程](../dev/api/controlling-a-workflow.md)

+++

+++ 如何更新資料庫結構？

修改結構描述後（新增欄位、建立表格、變更資料型別），透過&#x200B;**[!UICONTROL Tools > Advanced > Update database structure]**&#x200B;更新實體資料庫結構以套用變更。

**必要時：**&#x200B;新增欄位、建立/擴充表格、修改欄位屬性、新增/移除連結、建立索引。

**重要：**&#x200B;請先備份、在開發環境中測試、規劃停機時間以進行大型變更、與Adobe支援（受管理的Cloud Services）協調，請注意，某些變更可能會導致資料遺失。
...
Campaign會自動刪除連結至資料主體的所有資料：收件者設定檔、傳遞和追蹤記錄、具有擁有權關係的自訂資料，以及訂閱歷史記錄。
...
[隱私權管理](../start/privacy.md) | [報表](../reporting/gs-reporting.md)
...
* **[版本和升級常見問題集](upgrades.md)** — 檢查您的版本並瞭解升級相關資訊
...
* **[Experience League說明](https://experienceleague.adobe.com/en/browse/campaign/campaign-v8){target="_blank"}** - Campaign v8的說明資源（Web UI和CLient主控台）
