...
在2026年4月14日，法國資料保護機構資訊與自由全國委員會(CNIL)發佈了一項關於在電子郵件中使用追蹤畫素的[建議](https://www.cnil.fr/sites/default/files/2026-04/recommandation-pixels_de_suivi.pdf)。 此指引澄清何時需要同意，並強調正確同意實務對於電子郵件畫素追蹤的重要性。 此政策可能會影響任何實體傳送電子郵件給法國訂閱者的傳送實務。
...
* **傳遞的分類。** 使用emailType屬性（驗證、僅傳遞能力、異動、行銷、公共服務、B2B潛在客戶）擴充nms:delivery。 分類會未經同意而允許使用哪些畫素。
* **同意擷取。** 使用各用途同意結構擴充nms:recipient，其中包含字詞版本、時間戳記、擷取來源及到期日。 擴充登錄檔單和偏好設定中心，以便與電子郵件選擇加入分開收集畫素同意。
* **畫素發射。** 為每個畫素定義一個NmsTracking_OpenFormula （驗證、傳遞能力、效能、設定檔分析、詐騙偵測）。 傳遞型別規則會根據emailType和收件者的各用途同意，選取要發出的公式。 個人化區塊會封裝邏輯，使邏輯不會存在於個別創意中。
* **撤回。** 將「管理追蹤器偏好設定」連結新增至每個電子郵件頁尾，與取消訂閱連結不同。 連結指向透過idTracking驗證的nms:webApp登陸頁面；收件者只要按一下即可撤銷同意，無需重新輸入其電子郵件地址。 標準追蹤工作流程新增了篩選步驟，可防止在退出後重複開啟先前傳送的電子郵件。
* **同意證明。** 擷取每個同意事件在僅附加的記錄中（例如，pix:consentLog擴充功能名稱空間），並在字詞變更後個別儲存字詞版本以供擷取。 透過Adobe Campaign Explorer呈現記錄檔，並作為定期匯出。
* **重新請求治理。** lastPixelRevisionDate欄位和篩選型別規則可防止在拒絕後至少六個月重新請求。 定期工作流程有助於管理同意過期。
