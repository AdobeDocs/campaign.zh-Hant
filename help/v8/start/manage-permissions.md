...
1. **[!UICONTROL Administrator]** （管理員）
   ...
   * **[!UICONTROL ADMINISTRATION]**：執行/建立/編輯/刪除任何物件的權利，例如工作流程、傳遞、指令碼等
   ...
   >[!IMPORTANT]
   >
   >**[!UICONTROL Administrator]**&#x200B;角色授與Campaign控制面板的存取權。 Adobe Admin Console中任何名稱包含「管理員」字樣的產品設定檔（例如「管理員」、「管理員」、「管理員」、「核准管理員」等） 將授予「控制面板」的存取權。 深入瞭解[管理控制面板存取](https://experienceleague.adobe.com/docs/control-panel/using/discover-control-panel/managing-permissions.html?lang=zh-Hant){target="_blank"}。
