...
為了確保速度和規模兩方面的效能都更高，您可以允許傳遞在中間來源執行個體之間自動分割，以更快傳遞給收件者。 從行銷執行個體執行傳送時，此操作是透明的：傳送後，所有記錄會合併在一起，然後傳回行銷執行個體成為單一傳送物件。
...
* **執行執行個體**&#x200B;會擷取傳入的事件（例如密碼重設或網站訂單），並傳送個人化訊息。 可以有多個執行執行個體透過負載平衡器處理訊息，並調整要處理的事件數目以達最大可用性。
