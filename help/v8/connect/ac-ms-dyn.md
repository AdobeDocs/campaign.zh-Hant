...
在跨頻道通訊上啟用您的CRM資料：瞭解如何傳遞來自的連絡人 **Microsoft Dynamics 365** 到Adobe Campaign，並從Adobe Campaign將行銷活動效能資料（傳送、開啟、點按和彈回）分享回Microsoft Dynamics 365。

完成設定後，系統會透過專用工作流程活動在系統之間執行資料同步。 [了解更多](crm-data-sync.md)。

>[!NOTE]
>
>支援的Microsoft Dynamics版本在Campaign中有詳細說明 [相容性矩陣](../start/compatibility-matrix.md).

請依照下列步驟設定專用的外部帳戶，將Microsoft Dynamics 365資料匯入和匯出至Adobe Campaign。
...
1. 編碼私密金鑰。 [了解更多](#configure-acc-for-microsoftt)


### 取得Dynamics 365使用者端ID {#get-client-id-microsoft}

若要取得應用程式（使用者端） ID，您必須在Azure Active Directory中註冊應用程式。

1. 瀏覽至 **Azure Active Directory >應用程式註冊**，並選取 **新註冊**.
1. 輸入有助於識別執行個體的唯一名稱，例如 **adobecampaign`<instance identifier>`**.

儲存後，Microsoft Azure Directory會指派唯一的 **應用程式（使用者端） ID** 至您的應用程式。 稍後在Adobe Campaign中設定Dynamics 365時，您將需要此ID。
...
接著，您必須以base64編碼憑證。 要執行此操作，您可以使用Base64編碼器或命令列 `base64 -w0 private.key` 適用於Linux。

+++

### 設定許可權 {#config-permissions-microsoft}

**步驟1**：設定 **必要許可權** 適用於已建立的應用程式。

1. 導覽至 **Azure Active Directory >應用程式註冊** 並選取先前建立的應用程式。
1. 按一下 **設定** 左上角。
...
1. 選取您要授與租使用者範圍管理員同意的應用程式。
1. 從左窗格選單中選取 **許可權** 在 **安全性**.
1. 按一下 **授予管理員同意**.
...
1. 選取中的驗證方法 **[!UICONTROL CRM O-Auth type]** 下拉式清單。
   ...
   1. 若要設定Microsoft Dynamics CRM外部帳戶以與Adobe Campaign連線，請執行下列動作： **密碼認證**，提供下列詳細資料：

      * **伺服器**：您的Microsoft CRM伺服器的URL。 若要尋找您的Microsoft CRM伺服器URL，請存取您的Microsoft Dynamics CRM帳戶，然後按一下Dynamics 365並選取您的應用程式。 然後，您就可以在瀏覽器的位址列中找到您的伺服器URL，例如https://myserver.crm.dynamics.com/。
      * **帳戶**：用來登入Microsoft CRM的帳戶。
      * **密碼**：用來登入Microsoft CRM的帳戶。
      * **使用者端識別碼**：應用程式（使用者端） ID可從「更新程式碼類別，使用者端ID」欄位中的Microsoft Azure管理入口網站找到。
      * **CRM版本**：選擇Dynamics CRM 365 CRM版本。
   1. 若要設定Microsoft Adobe Campaign Dynamics CRM外部帳戶，以使用 **憑證**，提供下列詳細資料：
...
>[!NOTE]
>
>若要核准設定，請登出並重新登入Adobe Campaign使用者端主控台。
...
建立結構描述後，您就可以自動將列舉從Dynamics 365同步到Adobe Campaign。
...
| 屬性型別 | 支援 |
| --------------------------------------------------------------------------------- | --------- |
| 基本型別：布林值、datetime、decimal、float、double、integer、bigint、string | 是 |
| 金錢（雙倍） | 是 |
