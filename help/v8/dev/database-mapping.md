...
在 [企業(FFDA)部署](../architecture/enterprise-deployment.md),Adobe Campaign表格的主要索引鍵為 **通用唯一ID(UUID)** 由資料庫引擎自動產生。 鍵值在整個資料庫中是唯一的。 在插入記錄時自動生成密鑰的內容。
...
請注意， `autouuid="true"`參數會套用至 [企業(FFDA)部署](../architecture/enterprise-deployment.md) 只有。
