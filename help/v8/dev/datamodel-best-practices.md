...
如需深入了解Campaign內建表格及其彼此的關係，請參閱 [本節](datamodel.md) .

![](../assets/do-not-localize/glass.png) 閱讀 [本節](schemas.md) 以開始使用Campaign綱要。

![](../assets/do-not-localize/glass.png) 了解如何設定擴充功能結構，以擴充Adobe Campaign資料庫的概念資料模型，位於 [本頁](extend-schema.md).
...
>[!NOTE]
>
>Adobe Campaign可建置 [自訂收件者表格](custom-recipient.md). 不過，在大多數情況下，建議使用內建 [收件者表格](datamodel.md#ootb-profiles) 已預先建立其他表和功能。
...
* 此 **expr** 屬性可將方案屬性定義為計算欄位，而非表中的物理集值。 這可以讓使用者以不同格式（例如年齡和出生日期）存取資訊，而無須儲存這兩個值。 這是避免重複欄位的好方法。 例如，「收件者」表格會使用網域的運算式，而此運算式已出現在電子郵件欄位中。
* 不過，當運算式計算複雜時，不建議使用 **expr** 屬性即時計算可能會影響查詢的效能。
* 此 **XML** 類型是避免建立太多欄位的好方法。 但它也佔用了磁碟空間，因為它使用資料庫中的CLOB列。 它還可能導致複雜的SQL查詢，並可能影響效能。
* 長度 **字串** 欄應一律以欄定義。 依預設，Adobe Campaign中的最大長度為16K，但如果您已知道大小不會超過較短的長度，則Adobe建議將欄位保持在較短的長度。
...
除了 **autouuid** 和 **奧托普** 預設在大多數表中定義，您應考慮添加一些邏輯或業務密鑰（帳戶號、客戶端號等）。 它稍後可用於匯入/調解或資料包。 有關詳細資訊，請參閱 [識別碼](#identifiers).

高效的密鑰對效能至關重要。 透過Snowflake，您可以插入數值或字串式資料類型作為表格的索引鍵。

>[!NOTE]
>
>此 **autouuid** 屬性僅適用於 [企業(FFDA)部署](../architecture/enterprise-deployment.md).
...
| 名稱（或內部名稱） | <ul><li>此資訊是表中記錄的唯一標識符。 此值可手動更新，通常具有產生的名稱。</li><li>此識別碼會在部署至不同的Adobe Campaign例項時保留其值，且不應為空。</li></ul> | <ul><li>如果要將物件從環境部署至另一個環境，請重新命名Adobe Campaign產生的記錄名稱。</li><li>當對象具有命名空間屬性時(*綱要* 例如，此通用命名空間將用於所有建立的自訂物件。 不應使用某些保留的命名空間： *nms*, *xtk*、等  請注意，有些命名空間僅為內部。 [了解更多資訊](schemas.md#reserved-namespaces)。</li><li>當物件沒有任何命名空間時(*工作流程* 或 *傳遞* 例如)，此命名空間概念會新增為內部名稱物件的前置詞： *namespaceMyObjectName*.</li><li>請勿使用特殊字元，例如空格&quot; &quot;、半欄&quot;:&quot;或連字型大小&quot;-&quot;。 所有這些字元都將替換為底線「_」（允許的字元）。 例如，「abc-def」和「abc:def」會儲存為「abc_def」並互相覆寫。</li></ul> |
...
在 [企業(FFDA)部署](../architecture/enterprise-deployment.md), Adobe Campaign主要索引鍵是針對所有內建表格自動產生的UUID。 UUID也可用於自訂表格。 [了解更多](../architecture/keys.md)
...
* 使用主鍵作為外部系統主鍵。 此解決方案通常較為理想，因為它簡化了資料匯入和匯出的方法，且在不同系統之間具有一致的鍵值。 **Autouuid** 如果索引鍵名為「id」，且預期會填入外部值（非自動產生），則應停用。
...
將連結聲明為外部連接對效能不利。 零ID記錄模擬外部連接功能。 在 [企業(FFDA)部署](../architecture/enterprise-deployment.md)，若連結使用 **autouuid**.
...
如果用戶不能看到反向連結，則可以使用連結定義revLink=&#39;來隱藏它&#x200B;_無_&#39;。 例如，一個很好的使用案例是定義從收件者到最後完成交易的連結。 您只需查看從收件者到最後一個交易的連結，並且無需從交易表中看到任何反向連結。
...
![](../assets/do-not-localize/speech.png)  身為受管理Cloud Services使用者，請洽詢Adobe顧問或技術管理員，以進一步了解保留，或您是否需要為自訂表格設定保留。
...
* A **小尺寸** 表格類似於「傳送」表格。
* A **中等大小** 表格與收件者表格的大小相同。 每位客戶有一筆記錄。
* A **大尺寸** 表與「廣泛」日誌表類似。 每個客戶有許多記錄。
