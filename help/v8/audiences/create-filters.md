...
您可以存取以下欄位中的篩選器詳細資料： **[!UICONTROL Profiles and Targets > Pre-defined filters]** Explorer節點。
...
每個清單上方的欄位可讓您使用 **預先定義的預設篩選器** 以取得這份清單。 對於收件者清單，您可以依預設篩選名稱和電子郵件地址。
...
>[!NOTE]
>
>此 **%** 字元會取代任何字元字串。 例如，輸入 `%@gmail.com` 在電子郵件欄位中，顯示所有具有Gmail地址的設定檔。 輸入 `%@L` 在「姓氏」欄位中顯示所有姓氏為L的設定檔。
...
1. 瀏覽至收件者資料夾，然後按一下 **[!UICONTROL Remove this filter]** 圖示在目前篩選的右側：新的預設篩選可供使用。
   ![](assets/updated-default-filter.png)


## 快速篩選{#quick-filters}

使用和組合 **快速篩選** 以定義特定欄位的篩選器。
...
如果您需要對表單中不可用的欄位進行篩選，它會在欄中篩選，然後對該欄進行篩選。 若要這麼做，
...
在中合併複雜條件 **進階篩選**. 使用這些篩選器對您的資料建立複雜的查詢或查詢組合。 這些篩選器可以儲存並與其他Campaign使用者共用。
...
1. 有三種運運算元可用於組合運算式：  **與**， **或**， **除了**. 按一下箭頭以切換至 **或**.
...
1. 按一下篩選的說明以進行編輯。
1. 按一下 **[!UICONTROL Save as filter]** 圖示加以調整。
...
篩選器將新增到 [預先定義的篩選器](#predefined-filters). 可以從此節點更新。
...
儲存後，在中選擇訊息的目標母體時，可使用進階篩選器 **[!UICONTROL User filters]** 區段。
...
>[!NOTE]
>
>使用 **[!UICONTROL Exclude recipients from this segment]** 以僅鎖定不符合篩選條件的連絡人。
...
進階引數適用於預先定義的篩選器。 若要存取縮圖，請瀏覽至 **[!UICONTROL Parameters]** 的索引標籤中。

* 若要根據此「檔案」型別預設顯示所有清單的篩選，請選取 **[!UICONTROL Default filter for the associated document type]** 選項。

  例如， **[!UICONTROL By name or login]** 篩選器已套用至運運算元此選項已選取，因此篩選器一律會顯示在所有運運算元清單上。

* 若要讓篩選器可供所有Campaign運運算元使用，請選取  **[!UICONTROL Filter shared with other operators]** 選項。
